...
// @Description  This is used to check if the purchase would be successful so the frontend can show the user the result
// @Tags         products
// @Accept       json
// @Produce      json
// @Security     Bearer
...
// @Failure      400  {object}  ProductStandardErrorResponse
// @Failure      401  {object}  ProductStandardErrorResponse
// @Router       /events/{slug}/try-purchase [post]
func (h *ProductHandler) TryPurchaseProducts(w http.ResponseWriter, r *http.Request) {
	slug, err := extractSlugAndValidate(r)
	if err != nil {
		BadRequestError(w, err, "product")
		return
	...
	purchase_info, err := h.ProductService.TryPurchaseProducts(user, slug, reqBody)
	if err != nil {
		HandleErrMsg("error processing purchase", err, w).Stack("product").BadRequest()
		return
	}
